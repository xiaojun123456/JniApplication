...
int prepareCameraWithBase(JNIEnv *env,int width, int height, int pixFormat){
	int ret;

	pUsbCamera = (UsbCamera *) malloc(sizeof(UsbCamera));
	memset(pUsbCamera,'\0', sizeof(UsbCamera));
	UsbCamera *pCamera = pUsbCamera;
	sprintf(pCamera->deviceName, "/dev/video%d", 1);
